...
export function getGeneralRanking() {
  ...
  for (let n = 0; n < numCarreras; n++) {
    ...
    let datosN = RJson.map((racePilot) => {
      return { piloto: racePilot.name, tiempo: racePilot.races[n].time }; //obtenemos los tiempos de cada piloto para cada carrera, creando así un mapa más sencillo de trabajar
    });
    datosN.sort(
